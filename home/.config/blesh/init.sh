...
## [The setting "openat_base" needs to be set before ble.sh is initialized.
## Therefore the value should be specified to the source options as
## e.g. "source ble.sh -o openat_base=30" instead of using "bleopt" command.
## Another way has been setting the shell variable "bleopt_openat_base" before
## sourcing ble.sh, but this is no longer recommended because "bleopt_*" will
